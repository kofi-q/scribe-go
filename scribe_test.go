...
func TestFpdfImplementPdf(t *testing.T) {
	// this will not compile if Scribe and Tpl
	// do not implement Scriber
	var _ scribe.Scriber = (*scribe.Scribe)(nil)
	var _ scribe.Scriber = (*scribe.Tpl)(nil)
}
...
func TestPagedTemplate(t *testing.T) {
	...
	if tpl.NumPages() != 4 {
		t.Fatalf(
			"The template does not have the correct number of pages %d",
			tpl.NumPages(),
		)
	}
...
func TestIssue0193(t *testing.T) {
	...
	if err == nil {
		...
		_ = pdf.RegisterImageOptionsReader(
			"sweden",
			scribe.ImageOptions{ImageType: "png", ReadDpi: true},
			rdr,
		)
		err = pdf.Error()
	}
...
func TestIssue0209SplitLinesEqualMultiCell(t *testing.T) {
	...
	if int(y_end-y_start) != int(FontSize) {
		t.Fatalf(
			"expect MultiCell split in one line %.2f != %.2f",
			y_end-y_start,
			FontSize,
		)
	}
	...
	if int(y_end-y_start) != int(FontSize*2) {
		t.Fatalf(
			"expect MultiCell split in two lines %.2f != %.2f",
			y_end-y_start,
			FontSize*2,
		)
	}
...
func TestFooterFuncLpi(t *testing.T) {
	...
	if bytes.Contains(b, []byte(oldFooterFnc)) {
		t.Errorf(
			"not expecting %s render on pdf when FooterFncLpi is set",
			oldFooterFnc,
		)
	}
	...
	if got != 1 {
		t.Errorf(
			"footer %s should render only on first page got: %d",
			firstPageOnly,
			got,
		)
	}
	...
	if f > l {
		t.Errorf(
			"index %d (%s) should less than index %d (%s)",
			f,
			firstPageOnly,
			l,
			lastPageOnly,
