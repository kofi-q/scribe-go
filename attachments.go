...
type Attachment struct {
	...
	objectNumber uint32 // filled when content is included
}
...
func (f *Scribe) writeCompressedFileObject(content []byte) {
	...
	f.newobj()
	f.outf(
		"<< /Type /EmbeddedFile /Length %d /Filter /FlateDecode /Params << /CheckSum <%s> /Size %d >> >>\n",
		lenCompressed,
		sum,
		lenUncompressed,
	)
...
func (f *Scribe) embed(a *Attachment) {
...
func (f Scribe) getEmbeddedFiles() string {
...
type annotationAttach struct {
	*Attachment
	...
	x, y, w, h float32 // scribe-go coordinates (y diff and scaling done)
}
...
// demonstration of this method.
func (f *Scribe) AddAttachmentAnnotation(a *Attachment, x, y, w, h float32) {
	if a == nil {
		return
	}
	f.pageAttachments[f.page] = append(
		f.pageAttachments[f.page],
		annotationAttach{
			Attachment: a,
...
func (f *Scribe) putAnnotationsAttachments() {
	// avoid duplication
...
func (f *Scribe) putAttachmentAnnotationLinks(out *fmtBuffer, page int) {
	for _, an := range f.pageAttachments[page] {
		x1, y1, x2, y2 := an.x, an.y, an.x+an.w, an.y-an.h
		as := fmt.Sprintf(
			"<< /Type /XObject /Subtype /Form /BBox [%.2f %.2f %.2f %.2f] /Length 0 >>",
			x1,
			y1,
			x2,
			y2,
		...
		out.printf(
			"<< /Type /Annot /Subtype /FileAttachment /Rect [%.2f %.2f %.2f %.2f] /Border [0 0 0]\n",
			x1,
			y1,
			x2,
			y2,
