...
Package `scribe-go` implements a PDF document generator with high level support for
...
`scribe-go` has no dependencies other than the Go standard library. All tests pass
on Linux, Mac and Windows platforms.

`scribe-go` supports UTF-8 TrueType fonts and "right-to-left" languages. Note that
...
If an error occurs in an Scribe method, an internal error field is set. After
this occurs, Scribe method calls typically return without performing any
...
error to the Scribe instance by calling the `SetError()` method or the
`SetErrorf()` method. At any time during the life cycle of the Scribe instance,
...
example PDFs. These can be found in the scribe-go/pdf directory after the tests
complete.
...
with the same name as the example PDF is placed in the scribe-go/pdf/reference
...
can generate output in different forms. It uses scribe-go for its document
production mode.

## License

scribe-go is released under the MIT License. It is copyrighted by Kurt Jung and
...
templates; this allows templates to be stored independently of scribe-go. Paul
