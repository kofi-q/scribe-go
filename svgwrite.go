...
// paths.
func (f *Scribe) SVGBasicWrite(sb *SVGBasicType, scale float32) {
	originX, originY := f.GetXY()
	var x, y, newX, newY float32
	...
	sval := func(origin float32, arg int) float32 {
		return origin + scale*seg.Arg[arg]
	}
	xval := func(arg int) float32 {
		return sval(originX, arg)
	}
	yval := func(arg int) float32 {
		return sval(originY, arg)
	}
	val := func(arg int) (float32, float32) {
...
func (f *Scribe) SVGBasicDraw(sb *SVGBasicType, scale float32, styleStr string) {
	originX, originY := f.GetXY()
	...
	for j := 0; j < len(sb.Segments) && f.Ok(); j++ {
		...
		for k := 0; k < len(path) && f.Ok(); k++ {
			...
			case 'Z':
				f.ClosePath()
				f.DrawPath(cmp.Or(styleOverride, styleStr))
			default:
				f.SetErrorf("Unexpected path command '%c'", seg.Cmd)
			}
		}
