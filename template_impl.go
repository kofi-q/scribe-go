...
func newTpl(
	...
	bytes := make([][]byte, len(tpl.Scribe.pages))
	// skip the first page as it will always be empty
	for x := 1; x < len(bytes); x++ {
		bytes[x] = tpl.Scribe.pages[x].Bytes()
	}

	templates := make([]Template, 0, len(tpl.Scribe.templates))
	for _, key := range templateKeyList(tpl.Scribe.templates, true) {
...
func (t *FpdfTpl) Size() (corner PointType, size PageSize) {
...
func (t *FpdfTpl) FromPage(page int) (Template, error) {
	...
	if page == 0 {
		return nil, errors.New(
			"scribe-go: pages start at 1 No template will have a page 0",
		)
	}

	if page > t.NumPages() {
		return nil, fmt.Errorf(
			"scribe-go: the template does not have a page %d",
			page,
		)
	}
...
// Tpl is an Scribe used for writing a template. It has most of the facilities of
...
func (t *Tpl) loadParamsFromFpdf(f *Scribe) {
	t.Scribe.compress = false

	t.Scribe.k = f.k
	t.Scribe.x = f.x
	t.Scribe.y = f.y
	t.Scribe.lineWidth = f.lineWidth
	t.Scribe.capStyle = f.capStyle
	t.Scribe.joinStyle = f.joinStyle

	t.Scribe.color.draw = f.color.draw
	t.Scribe.color.fill = f.color.fill
	t.Scribe.color.text = f.color.text

	t.Scribe.fonts = f.fonts
	t.Scribe.currentFont = f.currentFont
	t.Scribe.fontSize = f.fontSize
	t.Scribe.fontSizePt = f.fontSizePt
	t.Scribe.fontStyle = f.fontStyle
	t.Scribe.ws = f.ws

	for key, value := range f.images {
		t.Scribe.images[key] = value
	}
}
