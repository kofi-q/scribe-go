# Scribe-Go - PDF document generator

_Forked from [go-pdf/fpdf](https://codeberg.org/go-pdf/fpdf) for personal use. Some of the original code has been substantially modified and may not work as expected/documented._

Package `scribe-go` implements a PDF document generator with high level support for text, drawing and images.
...
scribe-go supports UTF-8 TrueType fonts and “right-to-left” languages. Note
...
``` shell
go get github.com/kofi-q/scribe-go
```

Later, to receive updates, run

``` shell
go get -u -v github.com/kofi-q/scribe-go/...
```
...
``` go
doc := scribe.New("P", "mm", "A4", "")
doc.AddPage()
doc.SetFont("Arial", "B", 16)
doc.Cell(40, 10, "Hello, world")
err := doc.OutputFileAndClose("hello.pdf")
```

See the functions in the
[scribe-go\_test.go](https://github.com/kofi-q/scribe-go/blob/master/fpdf_test.go)
file (shown as examples in this documentation) for more advanced PDF
examples.

## Errors

If an error occurs in an Scribe method, an internal error field is set.
After this occurs, Scribe method calls typically return without performing
...
desirable for the application to transfer the error to the Scribe instance
by calling the `SetError()` method or the `SetErrorf()` method. At any
time during the life cycle of the Scribe instance, the error state can be
...
of example PDFs. These can be found in the scribe-go/pdf directory after
the tests complete.
...
the scribe-go/pdf/reference directory and if the third argument to
...
`scribe-go` is released under the MIT License. It is copyrighted by Kurt Jung and the contributors acknowledged below and in the [LICENSE](LICENSE) file.
