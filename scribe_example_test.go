...
import (
	...
	"github.com/kofi-q/scribe-go/internal/files"
)
...
// scribe-go library. If an error occurs at some point during the construction of
...
// application.
func Example() {
	pdf := scribe.New(scribe.OrientationPortrait, "mm", "A4", "")
	pdf.AddPage()
...
func ExampleFpdf_AddPage() {
	...
	pdf.SetHeaderFuncMode(func() {
		...
		pdf.SetY(5)
		pdf.SetFont("Arial", scribe.FontStyleB, 15)
		pdf.Cell(80, 0, "")
	...
	pdf.SetFooterFunc(func() {
		pdf.SetY(-15)
		pdf.SetFont("Arial", scribe.FontStyleI, 8)
		pdf.CellFormat(0, 10, fmt.Sprintf("Page %d/{nb}", pdf.PageNo()),
			"", 0, "C", false, 0, "")
	})
	pdf.AliasNbPages("")
	pdf.AddPage()
	pdf.SetFont("Times", scribe.FontStyleNone, 12)
	for j := 1; j <= 40; j++ {
...
func ExampleFpdf_MultiCell() {
	...
	chapterTitle := func(chapNum int, titleStr string) {
		// 	// Arial 12
		pdf.SetFont("Arial", scribe.FontStyleNone, 12)
		// Background color
	...
	chapterBody := func(fileStr string) {
		...
		pdf.Ln(-1)
		// Mention in italics
		pdf.SetFont("", scribe.FontStyleI, 0)
...
func ExampleFpdf_SplitLines_tables() {
	...
	for rowJ := 0; rowJ < 2; rowJ++ {
		...
		for colJ := 0; colJ < colCount; colJ++ {
			...
			cell.ht = float32(len(cell.list)) * lineHt
			if cell.ht > maxHt {
				maxHt = cell.ht
			}
			cellList[colJ] = cell
		...
		for colJ := 0; colJ < colCount; colJ++ {
			...
			for splitJ := 0; splitJ < len(cell.list); splitJ++ {
				...
				pdf.CellFormat(
					colWd-cellGap-cellGap,
					lineHt,
					string(cell.list[splitJ]),
					"",
					0,
					alignList[colJ],
					false,
...
func ExampleFpdf_CellFormat_tables() {
	...
	fancyTable := func() {
		...
		pdf.SetLineWidth(.3)
		pdf.SetFont("", scribe.FontStyleB, 0)
		// 	Header
		w := []float32{40, 35, 40, 45}
		wSum := 0.0
		for _, v := range w {
			wSum += v
		}
		...
		pdf.SetFont("", scribe.FontStyleNone, 0)
		// 	Data
		fill := false
		for _, c := range countryList {
			pdf.SetX(left)
...
func ExampleFpdf_HTMLBasicNew() {
	...
	pdf.AddPage()
	pdf.SetFont("Helvetica", scribe.FontStyleNone, 20)
	_, lineHt := pdf.GetFontSize()
	pdf.Write(lineHt, "To find out what's new in this tutorial, click ")
	pdf.SetFont("", scribe.FontStyleU, 0)
	link := pdf.AddLink()
...
func ExampleFpdf_AddFont() {
	pdf := scribe.New("P", "mm", "A4", example.FontDir())
	pdf.AddFont("Calligrapher", scribe.FontStyleNone, "calligra.json")
	pdf.AddPage()
	pdf.SetFont("Calligrapher", scribe.FontStyleNone, 35)
...
func ExampleFpdf_WriteAligned() {
	...
	pdf.AddPage()
	pdf.SetFont("Helvetica", scribe.FontStyleNone, 12)
	...
	pdf.SetWordSpacing(
		(pageWidth - pdf.GetStringWidth(line)) / float32(
			strings.Count(line, " "),
		),
	)
	pdf.WriteAligned(pageWidth, 35, line, "L")
	pdf.Ln(10)
	pdf.SetFont("Helvetica", scribe.FontStyleU, 12)
	pdf.WriteAligned(pageWidth, 35, line, "L")
	pdf.Ln(10)
	pdf.SetFont("Helvetica", scribe.FontStyleS, 12)
	pdf.WriteAligned(pageWidth, 35, line, "L")
...
func ExampleFpdf_Image() {
	...
	pdf.Image(
		example.ImageFile("logo-rgb.png"),
		10,
		100,
		30,
		0,
	...
	example.Summary(
		err,
		fileStr,
	) // FIXME(sbinet): use SummaryCompare. image embedding doesn't produce stable output.
	// Output:
...
func ExampleFpdf_SetAcceptPageBreakFunc() {
	...
	setCol := func(col int) {
		crrntCol = col
		x := margin + float32(col)*(colWd+gutter)
		pdf.SetLeftMargin(x)
		pdf.SetX(x)
	}
	pdf.SetHeaderFunc(func() {
		titleStr := "scribe-go"
		pdf.SetFont("Helvetica", scribe.FontStyleB, 48)
		wd := pdf.GetStringWidth(titleStr) + 6
...
func ExampleFpdf_SetKeywords() {
	...
	err = scribe.MakeFont(example.FontFile("CalligrapherRegular.pfb"),
	...
	if err == nil {
		...
		pdf.AddFont(
			...
			"CalligrapherRegular.json",
		)
		pdf.AddPage()
		pdf.SetFont("Calligrapher", scribe.FontStyleNone, 16)
...
func ExampleFpdf_SetAlpha() {
	...
	pdf.AddPage()
	pdf.SetFont("Helvetica", scribe.FontStyleNone, 18)
	pdf.SetXY(0, gapY)
	pdf.SetTextColor(0, 0, 0)
	pdf.CellFormat(
		pageW,
...
func ExampleFpdf_ClipText() {
	...
	pdf.SetFont("Helvetica", scribe.FontStyleNone, 24)
	pdf.SetXY(0, y)
	...
	y += 12
	pdf.SetFont("Helvetica", scribe.FontStyleB, 120)
	...
	pdf.ClipPolygon([]scribe.PointType{{X: 80, Y: y + 20}, {X: 90, Y: y},
		{X: 100, Y: y + 20}}, true)
...
func ExampleFpdf_PageSize() {
	pdf := scribe.NewCustom(&scribe.InitType{
		UnitStr:    "in",
		Size:       scribe.PageSize{Wd: 6, Ht: 6},
		FontDirStr: example.FontDir(),
	})
	pdf.SetMargins(0.5, 1, 0.5)
	pdf.SetFont("Times", scribe.FontStyleNone, 14)
	pdf.AddPageFormat("L", scribe.PageSize{Wd: 3, Ht: 12})
	pdf.SetXY(0.5, 1.5)
	...
	pdf.AddPageFormat("P", scribe.PageSize{Wd: 3, Ht: 12})
	pdf.SetXY(0.5, 6)
...
func ExampleFpdf_Bookmark() {
	pdf := scribe.New("P", "mm", "A4", "")
	pdf.AddPage()
	pdf.SetFont("Arial", scribe.FontStyleNone, 15)
...
func ExampleFpdf_TransformBegin() {
	...
	var refX, refY float32
	var refStr string
	pdf := scribe.New("P", "mm", "A4", "")
	pdf.AddPage()
	color := func(val uint8) {
		pdf.SetDrawColor(val, val, val)
		pdf.SetTextColor(val, val, val)
	}
	reference := func(str string, x, y float32, val uint8) {
		color(val)
		pdf.Rect(x, y, 40, 10, "D")
		pdf.Text(x, y-1, str)
	}
	refDraw := func(str string, x, y float32) {
		refStr = str
		refX = x
		refY = y
	...
	pdf.SetFont("Helvetica", scribe.FontStyleNone, titlePt)
...
func ExampleFpdf_RegisterImage() {
	...
	var infoPtr *scribe.ImageInfoType
	var imageFileStr string
	var imgWd, imgHt, lf, tp float32
	pdf := scribe.New("P", "mm", "A4", "")
	pdf.AddPage()
	pdf.SetMargins(10, 10, 10)
	pdf.SetFont("Helvetica", scribe.FontStyleNone, 15)
...
func ExampleFpdf_SplitLines() {
	...
	pdf := scribe.New("P", "mm", "A4", "") // A4 210.0 x 297.0
	pdf.SetFont("Times", scribe.FontStyleNone, fontPtSize)
	_, lineHt := pdf.GetFontSize()
	pdf.AddPage()
	pdf.SetMargins(10, 10, 10)
	lines := pdf.SplitLines([]byte(lorem()), wd)
	ht := float32(len(lines)) * lineHt
	y := (297.0 - ht) / 2.0
...
func ExampleFpdf_SVGBasicWrite() {
	...
	html := pdf.HTMLBasicNew()
	html.Write(lineHt, htmlStr)
	sig, err = scribe.SVGBasicFileParse(example.ImageFile(sigFileStr))
	if err == nil {
		scale := 100 / sig.Wd
		scaleY := 30 / sig.Ht
		if scale > scaleY {
...
func ExampleFpdf_CellFormat_align() {
	...
	var formatRect = func(pdf *scribe.Scribe, recList []recType) {
		linkStr := ""
		for pageJ := 0; pageJ < 2; pageJ++ {
			...
			linkStr = "https://github.com/kofi-q/scribe-go"
		}
	}
	pdf := scribe.New("P", "mm", "A4", "") // A4 210.0 x 297.0
	pdf.SetFont("Helvetica", scribe.FontStyleNone, 16)
...
// Windows-1252 code page (scribe-go default). See the example for CellFormat (4)
// for a way to do this automatically.
func ExampleFpdf_CellFormat_codepageescape() {
	pdf := scribe.New("P", "mm", "A4", "") // A4 210.0 x 297.0
	fontSize := 16.0
	pdf.SetFont("Helvetica", scribe.FontStyleNone, fontSize)
	...
	pdf.AddPage()
	htmlStr := `Until scribe-go supports UTF-8 encoded source text, source text needs ` +
	...
	pdf.Ln(2 * ht)
	write(
		"Voix ambigu\xeb d'un c\x9cur qui au z\xe9phyr pr\xe9f\xe8re les jattes de kiwi.",
	)
	write(
...
func ExampleFpdf_CellFormat_codepage() {
	pdf := scribe.New("P", "mm", "A4", example.FontDir()) // A4 210.0 x 297.0
	// See documentation for details on how to generate fonts
	pdf.AddFont("Helvetica-1251", scribe.FontStyleNone, "helvetica_1251.json")
	pdf.AddFont("Helvetica-1253", scribe.FontStyleNone, "helvetica_1253.json")
	fontSize := 16.0
	pdf.SetFont("Helvetica", scribe.FontStyleNone, fontSize)
	ht := pdf.PointConvert(fontSize)
	...
	pdf.AddPage()
	str := `scribe-go provides a translator that will convert any UTF-8 code point ` +
	...
	pdf.SetFont(
		...
		scribe.FontStyleNone,
		fontSize,
	) // Name matches one specified in AddFont()
	tr = pdf.UnicodeTranslatorFromDescriptor("cp1251")
	write("Съешь же ещё этих мягких французских булок, да выпей чаю.")

	pdf.SetFont("Helvetica-1253", scribe.FontStyleNone, fontSize)
...
func ExampleFpdf_SetProtection() {
	pdf := scribe.New("P", "mm", "A4", "")
	pdf.SetProtection(scribe.CnProtectPrint, "123", "abc")
	pdf.AddPage()
	pdf.SetFont("Arial", scribe.FontStyleNone, 12)
...
func ExampleFpdf_Polygon() {
	...
	const colCount = 4
	const ptSize = 36
	var x, y, radius, gap, advance float32
	var rgVal uint8
	var pts []scribe.PointType
	vertices := func(count int) (res []scribe.PointType) {
		var pt scribe.PointType
		res = make([]scribe.PointType, 0, count)
		mlt := 2.0 * math.Pi / float32(count)
		for j := 0; j < count; j++ {
			pt.Y, pt.X = math.Sincos(float32(j) * mlt)
			res = append(res, scribe.PointType{
				X: x + radius*pt.X,
				Y: y + radius*pt.Y})
		}
		return
	...
	pdf := scribe.New("P", "mm", "A4", "") // A4 210.0 x 297.0
	pdf.AddPage()
	pdf.SetFont("Helvetica", scribe.FontStyleNone, ptSize)
	pdf.SetDrawColor(0, 80, 180)
	gap = 12.0
	pdf.SetY(gap)
	pdf.CellFormat(190.0, gap, "Equilateral polygons", "", 1, "C", false, 0, "")
	radius = (210.0 - float32(colCount+1)*gap) / (2.0 * float32(colCount))
...
func ExampleFpdf_RegisterImageReader() {
	...
	const (
		...
		ht       = 297
		fontSize = 15
		urlStr   = "https://github.com/kofi-q/scribe-go/raw/main/image/scribe-go.png"
...
func ExampleFpdf_Beziergon() {
	...
	curveList := []scribe.PointType{
		srcPrev,
	} // point [, control 0, control 1, point]*
	control := func(x, y float32) {
		curveList = append(curveList, scribe.PointType{X: x, Y: y})
	}
	for j, src := range srcList {
		ctrl := ctrlList[jPrev]
	...
	pdf.SetDashPattern([]float32{0.8, 0.8}, 0)
	pdf.SetDrawColor(160, 160, 160)
	pdf.Polygon(srcList, "D")
	pdf.SetDashPattern([]float32{}, 0)
...
func ExampleFpdf_SetLineJoinStyle() {
	...
	pdf := scribe.New("L", "mm", "A4", "")
	pdf.AddPage()
	var draw = func(cap, join string, x0, y0, x1, y1 float32) {
...
func ExampleFpdf_DrawPath() {
	...
	var (
		polygon = func(cx, cy, r, n, dir float32) {
			da := 2 * math.Pi / n
			pdf.MoveTo(cx+r, cy)
			pdf.Text(cx+r, cy, "0")
			i := 1
		...
		polygons = func(cx, cy, r, n, dir float32) {
			d := 1.0
			for rf := r; rf > 0; rf -= 10 {
...
func ExampleFpdf_CreateTemplate() {
	...
	template := pdf.CreateTemplate(func(tpl *scribe.Tpl) {
		tpl.Image(example.ImageFile("logo.png"), 6, 6, 30, 0, false, "", 0, "")
		tpl.SetFont("Arial", scribe.FontStyleB, 16)
	...
	template2 := pdf.CreateTemplate(func(tpl *scribe.Tpl) {
		tpl.UseTemplate(template)
		subtemplate := tpl.CreateTemplate(func(tpl2 *scribe.Tpl) {
			tpl2.Image(
				example.ImageFile("logo.png"),
				6,
				86,
			...
			tpl2.SetFont("Arial", scribe.FontStyleB, 16)
	...
	template3, _ := scribe.DeserializeTemplate(b)

	pdf.AddPage()
	pdf.UseTemplate(template3)
	pdf.UseTemplateScaled(template3, scribe.PointType{X: 0, Y: 30}, tplSize)
	pdf.Line(40, 210, 60, 210)
	pdf.Text(40, 200, "Template example page 1")

	pdf.AddPage()
	pdf.UseTemplate(template2)
	pdf.UseTemplateScaled(
		template3,
		scribe.PointType{X: 0, Y: 30},
		tplSize.ScaleBy(1.4),
	)
...
func ExampleFpdf_AddFontFromBytes() {
	pdf := scribe.New("P", "mm", "A4", "")
	pdf.AddPage()
	pdf.AddFontFromBytes(
		"calligra",
		scribe.FontStyleNone,
		files.CalligraJson,
		files.CalligraZ,
	)
	pdf.SetFont("calligra", scribe.FontStyleNone, 16)
...
func ExampleFpdf_ClipRect() {
	...
	cols := []float32{60, 100, pagew - mleft - mright - 100 - 60}
	rows := [][]string{}
	for i := 1; i <= 50; i++ {
...
func ExampleFpdf_Rect() {
	...
	for _, row := range rows {
		...
		for i, txt := range row {
			...
			h := float32(len(lines))*lineHt + marginCell*float32(len(lines))
			if h > height {
				height = h
			}
		}
...
func ExampleFpdf_RegisterAlias() {
	...
	for i := 1; i <= numSections; i++ {
		pdf.Cell(
			0,
			10,
			fmt.Sprintf("Section %d begins on page {mark %d}", i, i),
		)
		pdf.Ln(10)
	}
	...
	for i := 1; i <= numSections; i++ {
		pdf.AddPage()
		pdf.RegisterAlias(
			fmt.Sprintf("{mark %d}", i),
			fmt.Sprintf("%d", pdf.PageNo()),
		)
		pdf.Write(
			10,
			fmt.Sprintf("Section %d, page %d of {nb}", i, pdf.PageNo()),
		)
	}
...
func ExampleFpdf_RegisterAlias_utf8() {
	...
	pdf.AddUTF8Font(
		"dejavu",
		scribe.FontStyleNone,
		example.FontFile("DejaVuSansCondensed.ttf"),
	)
	pdf.SetFont("dejavu", scribe.FontStyleNone, 12)
	...
	for i := 1; i <= numSections; i++ {
		pdf.Cell(
			0,
			10,
			fmt.Sprintf("Sekcio %d komenciĝas ĉe paĝo {ĉi tiu marko %d}", i, i),
		)
		pdf.Ln(10)
	}
	...
	for i := 1; i <= numSections; i++ {
		pdf.AddPage()
		pdf.RegisterAlias(
			fmt.Sprintf("{ĉi tiu marko %d}", i),
			fmt.Sprintf("%d", pdf.PageNo()),
		)
		pdf.Write(
			10,
			fmt.Sprintf("Sekcio %d, paĝo %d de {entute}", i, pdf.PageNo()),
		)
	}
...
func ExampleNewGrid() {
	...
	gr := scribe.NewGrid(13, 10, 187, 130)
	gr.TickmarksExtentX(0, 10, 4)
	gr.TickmarksExtentY(0, 10, 3)
	gr.Grid(pdf)

	gr = scribe.NewGrid(13, 154, 187, 128)
	...
	gr.XTickStr = func(val float32, precision int) string {
	...
	for month, val := range pts {
		dot(float32(month)+0.5, val)
	}
	pdf.SetDrawColor(255, 64, 64)
	pdf.SetAlpha(0.5, "Normal")
	pdf.SetLineWidth(1.2)
	gr.Plot(pdf, 0.5, 11.5, 50, func(x float32) float32 {
	...
	gr = scribe.NewGrid(13, 10, 187, 274)
...
func ExampleFpdf_SetPageBox() {
	...
	pdf := scribe.New("P", "mm", "A4", "") // 210mm x 297mm
	pdf.SetPageBox("crop", boxmargin, boxmargin, wd-2*boxmargin, ht-2*boxmargin)
	pdf.SetFont("Arial", scribe.FontStyleNone, pdf.UnitToPointConvert(fontsize))
	pdf.AddPage()
...
func ExampleFpdf_SubWrite() {
	...
	pdf := scribe.New("P", "mm", "A4", "") // 210mm x 297mm
	pdf.AddPage()
	pdf.SetFont("Arial", scribe.FontStyleNone, fontSize)
	_, lineHt := pdf.GetFontSize()
...
func ExampleFpdf_SetPage() {
	...
	pdf := scribe.New("L", "cm", "A4", "")
	pdf.SetFont("Times", scribe.FontStyleNone, 12)

	var time []float32
	temperaturesFromSensors := make([][]float32, 5)
	maxs := []float32{25, 41, 89, 62, 11}
	for i := range temperaturesFromSensors {
		temperaturesFromSensors[i] = make([]float32, 0)
	}

	for i := 0.0; i < 100; i += 0.5 {
		time = append(time, i)
		for j, sensor := range temperaturesFromSensors {
			dataValue := rnd.float32() * maxs[j]
			sensor = append(sensor, dataValue)
			temperaturesFromSensors[j] = sensor
		}
	}
	var graphs []scribe.GridType
	var pageNums []int
	...
	for i := range temperaturesFromSensors {
		...
		pdf.WriteAligned(
			0,
			0,
			"Temperature Sensor "+strconv.Itoa(i+1)+" (C) vs Time (min)",
			"C",
		)
		pdf.Ln(0.5)
		graph := scribe.NewGrid(pdf.GetX(), pdf.GetY(), 20, 10)
...
func ExampleFpdf_SetFillColor() {
	...
	draw := func(trX, trY float32) {
		...
		pdf.SetAlpha(0.5, "Normal")
		pdf.SetDashPattern([]float32{5, 10}, 0)
...
func ExampleFpdf_TransformRotate() {
	...
	pdf.SetHeaderFunc(func() {
		pdf.SetFont("Arial", scribe.FontStyleB, markFontHt)
...
func ExampleFpdf_AddUTF8Font() {
	...
	pdf.AddUTF8Font(
		"dejavu",
		scribe.FontStyleB,
		example.FontFile("DejaVuSansCondensed-Bold.ttf"),
	)
	pdf.AddUTF8Font(
		"dejavu",
		scribe.FontStyleI,
		example.FontFile("DejaVuSansCondensed-Oblique.ttf"),
	)
	pdf.AddUTF8Font(
		"dejavu",
		scribe.FontStyleB|scribe.FontStyleI,
		example.FontFile("DejaVuSansCondensed-BoldOblique.ttf"),
	)
	...
	if err == nil {
		...
		pdf.SetFont("dejavu", scribe.FontStyleB, 17)
		pdf.MultiCell(100, 8, "Text in different languages :", "", "C", false)
		pdf.SetFont("dejavu", scribe.FontStyleNone, 14)
		...
		if err == nil {
			...
			pdf.SetFont("dejavu", scribe.FontStyleB|scribe.FontStyleI, 17)
			pdf.MultiCell(
				100,
				8,
				"Greek text with alignStr = \"J\":",
				"",
				"C",
				false,
			)
			pdf.SetFont("dejavu", scribe.FontStyleI, 14)
...
func ExampleUTF8CutFont() {
	...
	if err == nil {
		...
		subFont = scribe.UTF8CutFont(fullFont, "abcde")
		...
		if err == nil {
			...
			pdf.AddPage()
			pdf.AddUTF8Font("calligra", scribe.FontStyleNone, subFontFileStr)
			pdf.SetFont("calligra", scribe.FontStyleNone, fontHt)
			write("cabbed")
			write("vwxyz")
			pdf.SetFont("courier", scribe.FontStyleNone, fontHt)
	...
	for row := 0; row < 4; row++ {
		...
		for col := 0; col < 4; col++ {
			...
			pdf.RoundedRect(
				...
				ht,
				radius,
				corner(row&1 == 1, row&2 == 2, col&1 == 1, col&2 == 2),
				"FD",
			)
			r += 8
			g += 10
...
func ExampleFpdf_SetUnderlineThickness() {
	pdf := scribe.New("P", "mm", "A4", "") // 210mm x 297mm
	pdf.AddPage()
	pdf.SetFont("Arial", scribe.FontStyleU, 12)
...
func ExampleFpdf_Cell_strikeout() {
	...
	for fontSize := 4; fontSize < 40; fontSize += 10 {
		pdf.SetFont("Arial", scribe.FontStyleS, float32(fontSize))
		pdf.SetXY(0, float32(fontSize))
		pdf.Cell(40, 10, "Hello World")
	}
...
func ExampleFpdf_SetTextRenderingMode() {
	...
	pdf.SetFont("Times", scribe.FontStyleNone, fontSz)
	pdf.Write(
		lineSz,
		"This document demonstrates various modes of text rendering. Search for \"Mode 3\" "+
...
func TestIssue0316(t *testing.T) {
	...
	pdf.AddUTF8FontFromBytes("dejavu", scribe.FontStyleNone, fontBytes)
	pdf.SetFont("dejavu", scribe.FontStyleNone, 16)
	pdf.Cell(40, 10, "Hello World!")
...
func ExampleFpdf_SetAttachments() {
	...
	a1 := scribe.Attachment{Content: file, Filename: "grid.go"}
	file, err = os.ReadFile("LICENSE")
	if err != nil {
		pdf.SetError(err)
	}
	a2 := scribe.Attachment{Content: file, Filename: "License"}
	pdf.SetAttachments([]scribe.Attachment{a1, a2})
	...
	summaryCompare(
		err,
		fileStr,
	) // FIXME(sbinet): SetAttachments doesn't produce stable output across *Nix/Windows.
	// Output:
...
func ExampleFpdf_AddOutputIntent() {
	...
	pdf.SetFont("Arial", scribe.FontStyleB, 16)
	pdf.Cell(40, 10, "Hello World!")
	pdf.AddOutputIntent(scribe.OutputIntentType{
		SubtypeIdent:              scribe.OutputIntent_GTS_PDFA1,
