...
// scribe-go and its various contributed packages located beneath the contrib
// directory.
package example
...
func init() {
	setRoot()
	scribe.SetDefaultCompression(false)
	scribe.SetDefaultCatalogSort(true)
	scribe.SetDefaultCreationDate(time.Date(2000, 1, 1, 0, 0, 0, 0, time.UTC))
	scribe.SetDefaultModificationDate(
...
func referenceCompare(fileStr string) (err error) {
	...
	if err == nil {
		...
		err = scribe.ComparePDFFiles(fileStr, refFileStr, false)
	}
	return
}
