...
func StateGet(pdf *Scribe) (st StateType) {
...
// specified by st.
func (st StateType) Put(pdf *Scribe) {
...
type TickFormatFncType func(val float32, precision int) string

// defaultFormatter returns the string form of val with precision decimal places.
func defaultFormatter(val float32, precision int) string {
	return strconv.FormatFloat(float64(val), 'f', precision, 32)
}
...
func linear(x1, y1, x2, y2 float32) (slope, intercept float32) {
	if x2 != x1 {
		slope = (y2 - y1) / (x2 - x1)
...
func linearTickmark(tm []float32, lo, hi float32) (slope, intercept float32) {
	ln := len(tm)
	if ln > 0 {
...
func NewGrid(x, y, w, h float32) (grid GridType) {
	grid.x = x
	grid.y = y
	grid.w = w
	grid.h = h
...
func (g GridType) WdAbs(dataWd float32) float32 {
	return float32(math.Abs(float64(g.xm * dataWd)))
}

// Wd converts dataWd, specified in logical data units, to the unit of measure
// specified in New().
func (g GridType) Wd(dataWd float32) float32 {
	return g.xm * dataWd
}

// XY converts dataX and dataY, specified in logical data units, to the X and Y
// position on the current page.
func (g GridType) XY(dataX, dataY float32) (x, y float32) {
...
func (g GridType) Pos(xRel, yRel float32) (x, y float32) {
	x = g.w*xRel + g.x
	y = g.h*(1-yRel) + g.y
	return
}
...
// edges.
func (g GridType) XRange() (min, max float32) {
	min = g.xTicks[0]
...
// edges.
func (g GridType) YRange() (min, max float32) {
	min = g.yTicks[0]
...
func (g *GridType) TickmarksContainX(min, max float32) {
...
// exact values of the tickmarks are to be set by the application.
func (g *GridType) TickmarksContainY(min, max float32) {
	g.yTicks, g.yPrecision = Tickmarks(min, max)
	g.ym, g.yb = linearTickmark(g.yTicks, g.y+g.h, g.y)
}

func extent(min, div float32, count int) (tm []float32, precision int) {
	tm = make([]float32, count+1)
	for j := 0; j <= count; j++ {
		tm[j] = min
		min += div
	}
...
func (g *GridType) TickmarksExtentX(min, div float32, count int) {
...
// viewer-friendly tickmarks are to be determined automatically.
func (g *GridType) TickmarksExtentY(min, div float32, count int) {
	g.yTicks, g.yPrecision = extent(min, div, count)
	g.ym, g.yb = linearTickmark(g.yTicks, g.y+g.h, g.y)
}

// func (g *GridType) SetXExtent(dataLf, paperLf, dataRt, paperRt float32) {
// 	g.xm, g.xb = linear(dataLf, paperLf, dataRt, paperRt)
// }

// func (g *GridType) SetYExtent(dataTp, paperTp, dataBt, paperBt float32) {
// 	g.ym, g.yb = linear(dataTp, paperTp, dataBt, paperBt)
// }

func lineAttr(pdf *Scribe, clr RGBAType, lineWd float32) {
...
func (g GridType) Grid(pdf *Scribe) {
	var st StateType
	var yLen, xLen int
	var textSz, halfTextSz, yMin, yMax, xMin, xMax, yDiv, xDiv float32
	var str string
	var strOfs, strWd, tp, bt, lf, rt, drawX, drawY float32
	...
	if xLen > 1 && yLen > 1 {
		...
		line := func(x1, y1, x2, y2 float32, heavy bool) {
			if heavy {
...
func (g GridType) Plot(
	pdf *Scribe,
	xMin, xMax float32,
	count int,
	fnc func(x float32) (y float32),
) {
	if count > 0 {
		var x, delta, drawX0, drawY0, drawX1, drawY1 float32
		delta = (xMax - xMin) / float32(count)
		x = xMin
		for j := 0; j <= count; j++ {
			if j == 0 {
				drawX1 = g.X(x)
