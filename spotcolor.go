...
// AddSpotColor adds an ink-based CMYK color to the scribe-go instance and
...
// color.
func (f *Scribe) AddSpotColor(nameStr string, c, m, y, k byte) {
	if f.err == nil {
		_, ok := f.spotColorMap[nameStr]
		if !ok {
			id := uint32(len(f.spotColorMap) + 1)
...
func (f *Scribe) getSpotColor(nameStr string) (clr spotColorType, ok bool) {
	if f.err == nil {
...
func (f *Scribe) SetDrawSpotColor(nameStr string, tint byte) {
	...
	if ok {
		...
		f.color.draw.str = sprintf(
			"/CS%d CS %.3f SCN",
			clr.id,
			float32(byteBound(tint))/100,
		)
		if f.page > 0 {
...
// The value for tint ranges from 0 (no intensity) to 100 (full intensity). It
// is quietly bounded to this range.
func (f *Scribe) SetFillSpotColor(nameStr string, tint byte) {
	var clr spotColorType
	var ok bool
...
// The value for tint ranges from 0 (no intensity) to 100 (full intensity). It
// is quietly bounded to this range.
func (f *Scribe) SetTextSpotColor(nameStr string, tint byte) {
	var clr spotColorType
	var ok bool
...
func (f *Scribe) returnSpotColor(clr colorType) (name string, c, m, y, k byte) {
...
// values are returned.
func (f *Scribe) GetDrawSpotColor() (name string, c, m, y, k byte) {
...
// values are returned.
func (f *Scribe) GetTextSpotColor() (name string, c, m, y, k byte) {
...
// are returned.
func (f *Scribe) GetFillSpotColor() (name string, c, m, y, k byte) {
	return f.returnSpotColor(f.color.fill)
}

func (f *Scribe) putSpotColors() {
...
func (f *Scribe) spotColorPutResourceDict() {
	f.out("/ColorSpace <<")
