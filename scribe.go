...
func scribeNew(
	...
	f.unitStr = unitStr
	f.defPageSize = cmp.Or(size, PageSizeA4)
	...
	f.gradientList = append(
		f.gradientList,
		gradientType{},
	) // gradientList[0] is unused
	// Set default PDF version number
	f.pdfVersion = pdfVers1_3
	f.SetProducer("scribe-go "+libVersion, true)
	f.layerInit()
	...
	// create a large enough buffer for formatting float32s.
...
// NewCustom returns a pointer to a new Scribe instance. Its methods are
...
func NewCustom(init *InitType) (f *Scribe) {
	return scribeNew(
		init.OrientationStr,
		init.UnitStr,
		init.Size,
...
// New returns a pointer to a new Scribe instance. Its methods are subsequently
...
func (f *Scribe) Ok() bool {
	return f.err == nil
}

// Err returns true if a processing error has occurred.
func (f *Scribe) Err() bool {
	return f.err != nil
}

// ClearError unsets the internal Scribe error. This method should be used with
...
func (f *Scribe) ClearError() {
	f.err = nil
}

// SetErrorf sets the internal Scribe error with formatted text to halt PDF
...
func (f *Scribe) SetErrorf(fmtStr string, args ...interface{}) {
	if f.err == nil {
		f.err = fmt.Errorf(fmtStr, args...)
	}
}

// String satisfies the fmt.Stringer interface and summarizes the Scribe
// instance.
func (f *Scribe) String() string {
	return "scribe-go v" + libVersion
}

// SetError sets an error to halt PDF generation. This may facilitate error
// handling by application. See also Ok(), Err() and Error().
func (f *Scribe) SetError(err error) {
	if f.err == nil && err != nil {
		f.err = err
	}
}

// Error returns the internal Scribe error; this will be nil if no error has occurred.
func (f *Scribe) Error() error {
	return f.err
}
...
// GetMargins()).
func (f *Scribe) GetPageSize() (width, height float32) {
	width = f.w
	height = f.h
	return
}
...
func (f *Scribe) GetMargins() (left, top, right, bottom float32) {
	left = f.lMargin
	top = f.tMargin
	right = f.rMargin
	bottom = f.bMargin
...
func (f *Scribe) SetMargins(left, top, right float32) {
	f.lMargin = left
	f.tMargin = top
	if right < 0 {
		right = left
...
func (f *Scribe) SetLeftMargin(margin float32) {
	f.lMargin = margin
...
func (f *Scribe) GetCellMargin() float32 {
	return f.cMargin
}
...
// New().
func (f *Scribe) SetCellMargin(margin float32) {
	f.cMargin = margin
}
...
func (f *Scribe) SetPageBoxRec(t string, pb PageBox) {
...
func (f *Scribe) SetPageBox(t string, x, y, wd, ht float32) {
...
func (f *Scribe) SetPage(pageNum int) {
...
// (see SetFontLocation()).
func (f *Scribe) SetFontLoader(loader FontLoader) {
	f.fontLoader = loader
}
...
func (f *Scribe) SetHeaderFuncMode(fnc func(), homeMode bool) {
	f.headerFnc = fnc
...
// should not be called directly by the application. The implementation in Scribe
// is empty, so you have to provide an appropriate function if you want page
// headers. fnc will typically be a closure that has access to the Scribe
...
func (f *Scribe) SetHeaderFunc(fnc func()) {
	f.headerFnc = fnc
}
...
// implementation in Scribe is empty, so you have to provide an appropriate
// function if you want page footers. fnc will typically be a closure that has
// access to the Scribe instance and other document generation variables. See
// SetFooterFuncLpi for a similar function that passes a last page indicator.
//
// This method is demonstrated in the example for AddPage().
func (f *Scribe) SetFooterFunc(fnc func()) {
	f.footerFnc = fnc
	f.footerFncLpi = nil
}
...
// implementation in Scribe is empty, so you have to provide an appropriate
// function if you want page footers. fnc will typically be a closure that has
// access to the Scribe instance and other document generation variables.
func (f *Scribe) SetFooterFuncLpi(fnc func(lastPage bool)) {
	f.footerFncLpi = fnc
	f.footerFnc = nil
}
...
func (f *Scribe) SetTopMargin(margin float32) {
	f.tMargin = margin
}

// SetRightMargin defines the right margin. The method can be called before
// creating the first page.
func (f *Scribe) SetRightMargin(margin float32) {
	f.rMargin = margin
}
...
func (f *Scribe) GetAutoPageBreak() (auto bool, margin float32) {
...
// is 2 cm.
func (f *Scribe) SetAutoPageBreak(auto bool, margin float32) {
...
func (f *Scribe) GetDisplayMode() (zoomStr, layoutStr string) {
...
// viewer default mode.
func (f *Scribe) SetDisplayMode(zoomStr, layoutStr string) {
	if f.err != nil {
		return
	}
	if layoutStr == "" {
...
func (f *Scribe) GetCompression() bool {
	return f.compress
}
...
func (f *Scribe) SetCompression(compress bool) {
	f.compress = compress
}

// GetProducer returns the producer of the document as ISO-8859-1 or UTF-16BE.
func (f *Scribe) GetProducer() string {
	return f.producer
}
...
func (f *Scribe) GetSubject() string {
	return f.subject
}
...
func (f *Scribe) GetAuthor() string {
	return f.author
}
...
func (f *Scribe) GetCreator() string {
	return f.creator
}
...
func (f *Scribe) AliasNbPages(aliasStr string) {
	if aliasStr == "" {
		aliasStr = "{nb}"
	}
...
// AddPage().
func (f *Scribe) PageSize(pageNum int) (wd, ht float32, unitStr string) {
...
func (f *Scribe) AddPageFormat(orientationStr string, size PageSize) {
	if f.err != nil {
		return
	}
...
//
// See the example for AddPage() for a demonstration of this method.
func (f *Scribe) PageNo() int {
	return f.page
}

func colorComp(v uint8) (uint8, float32) {
	return v, float32(v) / 255.0
}

func (f *Scribe) rgbColorValue(
	r, g, b uint8,
	grayStr, fullStr string,
) (clr colorType) {
	...
	if len(grayStr) > 0 {
		if clr.gray {
			// clr.str = sprintf("%g %s", clr.r, grayStr)
			f.fmt.col.Reset()
			...
			// clr.str = sprintf("%g %g %g %s", clr.r, clr.g, clr.b, fullStr)
			f.fmt.col.Reset()
			f.fmt.col.WriteString(f.fmtF64(clr.r, prec))
			f.fmt.col.WriteString(" ")
		...
		// clr.str = sprintf("%g %g %g", clr.r, clr.g, clr.b)
		f.fmt.col.Reset()
		f.fmt.col.WriteString(f.fmtF64(clr.r, prec))
		f.fmt.col.WriteString(" ")
		f.fmt.col.WriteString(f.fmtF64(clr.g, prec))
...
func (f *Scribe) GetDrawColor() (uint8, uint8, uint8) {
...
// (for example, spot) has been more recently set.
func (f *Scribe) GetFillColor() (uint8, uint8, uint8) {
...
func (f *Scribe) SetTextColor(r, g, b uint8) {
...
// (for example, spot) has been more recently set.
func (f *Scribe) GetTextColor() (uint8, uint8, uint8) {
...
func (f *Scribe) GetLineWidth() float32 {
	return f.lineWidth
}

// GetLineCapStyle returns the current line cap style.
func (f *Scribe) GetLineCapStyle() string {
	switch f.capStyle {
	case 1:
		return "round"
	case 2:
...
// SetLineCapStyle defines the line cap style. style should be "butt",
...
// retained from page to page.
func (f *Scribe) SetLineCapStyle(style string) {
	var capStyle int
	switch style {
	case "round":
		capStyle = 1
...
func (f *Scribe) GetLineJoinStyle() string {
	switch f.joinStyle {
	case 1:
		return "round"
	case 2:
...
func (f *Scribe) SetDashPattern(dashArray []float32, dashPhase float32) {
	scaled := make([]float32, len(dashArray))
...
func (f *Scribe) outputDashPattern() {
	...
	for i, value := range f.dashArray {
		...
		buf.WriteString(strconv.FormatFloat(float64(value), 'f', 2, 32))
	}
	buf.WriteString("] ")
	buf.WriteString(strconv.FormatFloat(float64(f.dashPhase), 'f', 2, 32))
	buf.WriteString(" d")
	f.outbuf(&buf)
}
...
//
// It can be drawn (border only), filled (with no border) or both. style can
...
// (with no border) or both. style can be "F" for filled, "D" for outlined
...
func (f *Scribe) RoundedRect(
	x, y, w, h, r float32,
	corners string,
	stylestr string,
) {
	// This routine was adapted by Brigham Thompson from a script by Christophe Prugnaud
	var rTL, rTR, rBR, rBL float32 // zero means no rounded corner
...
// RoundedRect() example.
func (f *Scribe) RoundedRectExt(
	x, y, w, h, rTL, rTR, rBR, rBL float32,
	stylestr string,
) {
...
//
// style can be "F" for filled, "D" for outlined only, or "DF" or "FD" for
...
func (f *Scribe) Circle(x, y, r float32, style string) {
	f.Ellipse(x, y, r, r, 0, style)
}
...
func (f *Scribe) Ellipse(x, y, rx, ry, degRotate float32, style string) {
	f.arc(x, y, rx, ry, degRotate, 0, 360, style, false)
}
...
// the current draw color and line width centered on the ellipse's perimeter.
// Filling uses the current fill color.
func (f *Scribe) Polygon(points []PointType, style string) {
	if len(points) > 2 {
		const prec = -1
...
// the current draw color and line width centered on the ellipse's perimeter.
// Filling uses the current fill color.
func (f *Scribe) Beziergon(points []PointType, style string) {
...
func (f *Scribe) CurveCubic(
	x0, y0, cx0, cy0, x1, y1, cx1, cy1 float32,
	style string,
) {
	// f.point(x0, y0)
...
//
// The Circle() example demonstrates this method.
func (f *Scribe) CurveBezierCubic(
...
// The Circle() example demonstrates this method.
func (f *Scribe) Arc(
	x, y, rx, ry, degRotate, degStart, degEnd float32,
	style string,
) {
	f.arc(x, y, rx, ry, degRotate, degStart, degEnd, style, false)
}
...
// details.
func (f *Scribe) GetAlpha() (alpha float32, blendModeStr string) {
...
func (f *Scribe) SetAlpha(alpha float32, blendModeStr string) {
	if f.err != nil {
		return
	}
	...
	if alpha < 0.0 || alpha > 1.0 {
		f.err = fmt.Errorf(
			"alpha value (0.0 - 1.0) is out of range: %g",
			alpha,
		)
		return
	}
	...
	if !ok {
		...
		f.blendList = append(
			f.blendList,
			blendModeType{alphaStr, alphaStr, blendModeStr, 0},
		)
...
func (f *Scribe) gradientClipStart(x, y, w, h float32) {
	{
		const prec = -1
...
func (f *Scribe) gradientClipEnd() {
	// Restore previous graphic state
	f.out("Q")
}

func (f *Scribe) gradient(
	tp int,
...
//
// This ClipText() example demonstrates this method.
func (f *Scribe) ClipRoundedRect(x, y, w, h, r float32, outline bool) {
...
func (f *Scribe) ClipRoundedRectExt(
	x, y, w, h, rTL, rTR, rBR, rBL float32,
	outline bool,
) {
	f.clipNest++
...
//
// This ClipText() example demonstrates this method.
func (f *Scribe) ClipEllipse(x, y, rx, ry float32, outline bool) {
	f.clipNest++
...
func (f *Scribe) ClipCircle(x, y, r float32, outline bool) {
...
//
// The ClipText() example demonstrates this method.
func (f *Scribe) ClipPolygon(points []PointType, outline bool) {
	f.clipNest++
	var s fmtBuffer
	h := f.h
	k := f.k
...
func makeSubsetRange(set *bitset.BitSet, end uint32) {
	for i := range end {
		set.Set(uint(i))
	}
}
...
// file cannot be read.
//
// family specifies the font family. It can be either a name defined by
...
//
// style can be "B" (bold), "I" (italic), "U" (underscore), "S" (strike-out)
...
func (f *Scribe) StyleAdd(style FontStyle) {
	f.SetFont(f.currentFont, f.fontStyle|style, f.fontSizePt)
}

func (f *Scribe) StyleRemove(style FontStyle) {
	f.SetFont(f.currentFont, f.fontStyle&^style, f.fontSizePt)
}
...
func (f *Scribe) SetMeasurementFont(
	...
	prev := f.measurementFont
	if (id == 0 || id == prev.id) &&
		(style == prev.fontStyle) &&
		(size == 0 || size == prev.fontSizePt) {
		return
	}

	f.measurementFont = f.getFont(id, style, size)
}
...
func (f *Scribe) GetFontStyle() FontStyle {
	return f.fontStyle
}

// SetFontStyle sets the style of the current font. See also SetFont()
func (f *Scribe) SetFontStyle(style FontStyle) {
...
func (f *Scribe) CurrentFont() (FontId, FontStyle, float32) {
	return f.currentFont, f.fontStyle, f.fontSizePt
}
...
func (f *Scribe) GetFontSize() (ptSize, unitSize float32) {
...
func (f *Scribe) newLink(x, y, w, h float32, link int, linkStr string) {
...
// returned by AddLink().
func (f *Scribe) Link(x, y, w, h float32, link int) {
...
// is the target URL.
func (f *Scribe) LinkString(x, y, w, h float32, linkStr string) {
...
func (f *Scribe) Bookmark(txtStr string, level int, y float32) {
	if y == -1 {
		y = f.y
	}
...
func (f *Scribe) GetWordSpacing() float32 {
	return f.ws
}
...
func (f *Scribe) SetTextRenderingMode(mode int) {
	if mode >= 0 && mode <= 7 {
...
func (f *Scribe) SetAcceptPageBreakFunc(fnc func() bool) {
	...
	if len(txtStr) > 0 {
		...
		case strings.Contains(alignStr, "A"):
			...
			if d.Descent == 0 {
				...
				descent = d.Descent*f.fontSize/d.Ascent - d.Descent
			}
			dy = (height-f.fontSize)/2.0 - descent
		default:
			dy = 0
		}
		if f.colorFlag {
...
func (f *Scribe) Cell(w, h float32, txtStr string) {
...
func (f *Scribe) Cellf(w, h float32, fmtStr string, args ...interface{}) {
...
// simple way.
func (f *Scribe) SplitLines(txt []byte, w float32) [][]byte {
	// Function contributed by Bruno Michel
	lines := [][]byte{}
	font := f.font()
	wmax := int(math.Ceil(float64((w - 2*f.cMargin) * 1000 / f.fontSize)))
	s := bytes.Replace(txt, []byte("\r"), []byte{}, -1)
	strlen := len(s)
	for strlen > 0 && s[strlen-1] == '\n' {
		strlen--
	}
	s = s[0:strlen]
	sep := -1
	...
	for i < strlen {
		c := s[i]
		_, width := font.GlyphWidth(rune(c))
		l += int(width)
		if c == ' ' || c == '\t' || c == '\n' {
			sep = i
		}
...
func (f *Scribe) MultiCell(
	...
	for i < runeCount {
		...
		_, charWidth := font.GlyphWidth(c)
		if charWidth != 65535 { //Marker width 65535 used for zero width symbols
			l += charWidth
		}
		if l > wmax {
			...
			if sep == -1 {
				...
				if alignStr == "J" {
					if ns > 1 {
						f.ws = float32((wmax-ls)/1000) * f.fontSize / float32(ns-1)
					} else {
						f.ws = 0
					}
...
func (f *Scribe) write(lnHeight float32, txt string, link int, linkStr string) {
	...
	for i < nb {
		...
		_, glyphWidth := font.GlyphWidth(c)
		l += glyphWidth
		if l > wmax {
			// Automatic line break
			if sep == -1 {
...
func (f *Scribe) Write(h float32, txtStr string) {
	f.write(h, txtStr, 0, "")
}
...
func (f *Scribe) Writef(h float32, fmtStr string, args ...interface{}) {
...
func (f *Scribe) WriteLinkString(h float32, displayStr, targetStr string) {
...
// details.
func (f *Scribe) WriteLinkID(h float32, displayStr string, linkID int) {
...
func (f *Scribe) WriteAligned(
	width, lineHeight float32,
...
func (f *Scribe) ImageTypeFromMime(mimeStr string) (tp string) {
	switch mimeStr {
	case "image/png":
		tp = "png"
	case "image/jpg":
...
func (f *Scribe) imageOut(
	...
	// Flowing mode
	if flow {
		if f.y+h > f.pageBreakTrigger && !f.inHeader && !f.inFooter &&
			f.acceptPageBreak() {
			// Automatic page break
			x2 := f.x
			f.AddPageFormat(f.curOrientation, f.curPageSize)
...
func (f *Scribe) RegisterImageReader(
	imgName, tp string,
	r io.Reader,
) (info *ImageInfoType) {
	options := ImageOptions{
...
func (f *Scribe) RegisterImageOptionsReader(
	imgName string,
	options ImageOptions,
	r io.Reader,
) (info *ImageInfoType) {
	...
	if options.ImageType == "" {
		f.err = fmt.Errorf(
			"image type should be specified if reading from custom reader",
		)
		return
	}
...
func (f *Scribe) RegisterImage(fileStr, tp string) (info *ImageInfoType) {
	options := ImageOptions{
		ReadDpi:   false,
		ImageType: tp,
	}
...
func (f *Scribe) RegisterImageOptions(
	fileStr string,
	options ImageOptions,
) (info *ImageInfoType) {
	...
	if options.ImageType == "" {
		...
		if pos < 0 {
			f.err = fmt.Errorf(
				"image file has no extension and no type was specified: %s",
				fileStr,
			)
			return
		}
...
func (f *Scribe) GetImageInfo(imageStr string) (info *ImageInfoType) {
	return f.images[imageStr]
}

// ImportObjects imports objects from gofpdi into current document
func (f *Scribe) ImportObjects(objs map[string][]byte) {
	for k, v := range objs {
		f.importedObjs[k] = v
	}
}
...
func (f *Scribe) ImportObjPos(objPos map[string]map[uint32]string) {
...
func (f *Scribe) putImportedTemplates() {
	nOffset := f.n + 1
...
// PDF page onto page.
func (f *Scribe) UseImportedTemplate(
	tplName string,
	scaleX float32,
	scaleY float32,
	tX float32,
...
func (f *Scribe) ImportTemplates(tpls map[string]string) {
...
// creating the PDF.
func (f *Scribe) GetConversionRatio() float32 {
	return f.k
}
...
// cell margin.
func (f *Scribe) GetXY() (float32, float32) {
	return f.x, f.y
}
...
// the page.
func (f *Scribe) SetY(y float32) {
	// dbg("SetY x %g, lMargin %g", f.x, f.lMargin)
	f.x = f.lMargin
	if y >= 0 {
		f.y = y
	} else {
...
// bottom of the page.
func (f *Scribe) SetXY(x, y float32) {
	f.SetY(y)
	f.SetX(x)
}
...
func (f *Scribe) SetProtection(
	actionFlag byte,
	userPassStr, ownerPassStr string,
) {
	if f.err != nil {
		return
	}
...
// is produced.
func (f *Scribe) OutputAndClose(w io.WriteCloser) error {
	_ = f.Output(w)
	err := w.Close()
	if err != nil {
...
func (f *Scribe) OutputFileAndClose(fileStr string) error {
	if f.err != nil {
		return f.err
	}
...
func (f *Scribe) Output(w io.Writer) error {
	if f.err != nil {
		return f.err
	}
	// dbg("Output")
...
func (f *Scribe) beginpage(orientationStr string, size PageSize) {
	if f.err != nil {
		return
	}
	f.page++
	...
	f.x = f.lMargin
	f.y = f.tMargin
	// f.fontFamily = "" // [TODO] Figure out why this was necessary before
	...
	if orientationStr != f.curOrientation || size.Wd != f.curPageSize.Wd ||
		size.Ht != f.curPageSize.Ht {
	...
	if orientationStr != f.defOrientation || size.Wd != f.defPageSize.Wd ||
		size.Ht != f.defPageSize.Ht {
		f.pageSizes[f.page] = PageSize{f.wPt, f.hPt}
	}
}

func (f *Scribe) endpage() {
	f.EndLayer()
	f.state = 1
}

// Escape special characters in strings
func (f *Scribe) escape(s string) string {
...
func (f *Scribe) textstring(s string) string {
...
func (f *Scribe) GetUnderlineThickness() float32 {
...
func (f *Scribe) SetUnderlineThickness(thickness float32) {
...
func (f *Scribe) parsejpg(r io.Reader) (info *ImageInfoType) {
	info = f.newImageInfo()
	var (
		data bytes.Buffer
		err  error
	...
	info.w = float32(config.Width)
	info.h = float32(config.Height)
	info.f = "DCTDecode"
	info.bpc = 8
...
// correctly.
func (f *Scribe) RawWriteStr(str string) {
	f.out(str)
}
...
func (f *Scribe) RawWriteBuf(r io.Reader) {
	f.outbuf(r)
}
...
func (f *Scribe) fmtF64(v float32, prec int) string {
	return string(strconv.AppendFloat(f.fmt.buf[:0], float64(v), 'f', prec, 32))
}

func (f *Scribe) putInt(v int) {
	f.put(f.fmtInt(v))
}

func (f *Scribe) fmtInt(v int) string {
...
// will be used when initializing a new Scribe instance. See SetCatalogSort() for
// more details.
...
func (f *Scribe) GetCatalogSort() bool {
...
func (f *Scribe) SetCatalogSort(flag bool) {
	f.catalogSort = flag
}

// SetDefaultCreationDate sets the default value of the document creation date
// that will be used when initializing a new Scribe instance. See
...
func (f *Scribe) GetCreationDate() time.Time {
...
// previously defined.
func (f *Scribe) GetJavascript() string {
	if f.javascript == nil {
		return ""
	}
	return *f.javascript
...
func (f *Scribe) SetJavascript(script string) {
	f.javascript = &script
}
...
func (f *Scribe) putpages() {
	...
	pagesObjectNumbers := make([]uint32, nb+1) // 1-based
	for n := 1; n <= nb; n++ {
		// Page
		f.newobj()
		...
		if len(f.pageLinks[n])+len(f.pageAttachments[n]) > 0 {
			...
			for _, pl := range f.pageLinks[n] {
				annots.printf(
					"<</Type /Annot /Subtype /Link /Rect [%g %g %g %g] /Border [0 0 0] ",
					pl.x,
					pl.y,
					pl.x+pl.wd,
					pl.y-pl.ht,
	...
	}
	{
		...
		for id := range f.fonts.Len() {
			...
			case "UTF8":
				...
				{
					lastChar := uint16(usedRunes[0])
					f.put("/W [ " + strconv.Itoa(int(lastChar)) + " [ ")
					for _, c := range usedRunes {
						char := uint16(c)
						if char < lastChar || char-lastChar > 1 {
				...
				f.out("/CIDToGIDMap " + strconv.Itoa(int(f.n)+2) + " 0 R>>")
				f.out("endobj")
				...
				cidToGidMapCompressed := mem.bytes()
				f.newobj()
				f.out(
					"<</Length " + strconv.Itoa(
						len(cidToGidMapCompressed),
					) + "/Filter /FlateDecode>>",
				)
				f.putstream(cidToGidMapCompressed)
				f.out("endobj")
				mem.release()
...
func (f *Scribe) putimage(info *ImageInfoType) {
	f.newobj()
	info.n = f.n
	f.out("<</Type /XObject")
	f.out("/Subtype /Image")
	...
	if len(info.smask) > 0 {
		smask := &ImageInfoType{
			...
			f:   info.f,
			dp: sprintf(
				"/Predictor 15 /Colors 1 /BitsPerComponent 8 /Columns %d",
				int(info.w),
			),
			data:  info.smask,
			scale: f.k,
...
func (f *Scribe) putGradients() {
	...
	for j := 1; j < count; j++ {
		...
		if gr.tp == 2 || gr.tp == 3 {
			f.newobj()
			f.outf(
				"<</FunctionType 2 /Domain [0.0 1.0] /C0 [%s] /C1 [%s] /N 1>>",
				gr.clr1Str,
				gr.clr2Str,
			)
			f.out("endobj")
		...
		case 2:
			f.outf(
				"/Coords [%g %g %g %g] /Function %d 0 R /Extend [true true]>>",
				gr.x1,
				gr.y1,
				gr.x2,
				gr.y2,
		...
		case 3:
			f.outf(
				"/Coords [%g %g 0 %g %g %g] /Function %d 0 R /Extend [true true]>>",
				gr.x1,
				gr.y1,
				gr.x2,
				gr.y2,
...
func (f *Scribe) putcatalog() {
	...
	// 		$this->out('/OpenAction [3 0 R /XYZ null null '.sprintf('%g',$this->zoomMode/100).']');
	switch f.layoutMode {
...
func (f *Scribe) putbookmarks() {
	...
	if nb > 0 {
		...
		for _, o := range f.outlines {
			...
			f.outf("/Parent %d 0 R", n+uint32(o.parent))
			if o.prev > 0 {
				f.outf("/Prev %d 0 R", n+uint32(o.prev))
			}
			if o.next > 0 {
				f.outf("/Next %d 0 R", n+uint32(o.next))
			}
			if o.first > 0 {
				f.outf("/First %d 0 R", n+uint32(o.first))
			}
			if o.last > 0 {
		...
		f.outf("/Last %d 0 R>>", n+uint32(lru[0]))
		f.out("endobj")
	}
}

func (f *Scribe) putOutputIntents() {
	...
	for index, oi := range f.outputIntents {
		...
		f.outf(
			...
			oi.SubtypeIdent,
			oi.OutputConditionIdentifier,
			infoSegment,
			f.outputIntentStartN+uint32(index),
		)
	}
	f.out("]")
}

func (f *Scribe) putOutputIntentStreams() {
	if len(f.outputIntents) <= 0 {
		return
	}
	...
	for _, oi := range f.outputIntents {
		...
		f.outf(
			"<< /N 3 /Alternate /DeviceRGB /Length %d /Filter /FlateDecode >>",
			len(compressedICC),
		)
...
// using the path drawing routines rather than multiple Scribe.Line is
// that PDF creates nice line joins at the angles, rather than just
// overlaying the lines.
func (f *Scribe) MoveTo(x, y float32) {
	f.point(x, y)
	f.x, f.y = x, y
}
...
//
// The MoveTo() example demonstrates this method.
func (f *Scribe) CurveBezierCubicTo(cx0, cy0, cx1, cy1, x, y float32) {
...
//
// The MoveTo() example demonstrates this method.
func (f *Scribe) DrawPath(style string) {
	f.outf("%s", fillDrawOp(style))
}
...
//
// The MoveTo() example demonstrates this method.
func (f *Scribe) ArcTo(x, y, rx, ry, degRotate, degStart, degEnd float32) {
	f.arc(x, y, rx, ry, degRotate, degStart, degEnd, "", true)
}

func (f *Scribe) arc(x, y, rx, ry, degRotate, degStart, degEnd float32,
	style string, path bool) {
	x *= f.k
	y = (f.h - y) * f.k
	rx *= f.k
	ry *= f.k
	...
	dt := angleTotal / float32(segments)
	dtm := dt / 3
	if degRotate != 0 {
		a := -degRotate * math.Pi / 180
		sin, cos := math.Sincos(float64(a))
		//	f.outf("q %g %g %g %g %g %g cm",
		//		math.Cos(a), -1*math.Sin(a),
		//		math.Sin(a), math.Cos(a), x, y)
		const prec = -1
		f.put("q ")
		f.putF64(float32(cos), prec)
		f.put(" ")
		f.putF64(float32(-1*sin), prec)
		f.put(" ")
		f.putF64(float32(sin), prec)
		f.put(" ")
		f.putF64(float32(cos), prec)
		f.put(" ")
		f.putF64(x, prec)
	...
	t := angleStart
	a0 := x + rx*float32(math.Cos(float64(t)))
	b0 := y + ry*float32(math.Sin(float64(t)))
	c0 := -rx * float32(math.Sin(float64(t)))
	d0 := ry * float32(math.Cos(float64(t)))
	...
	for j := 1; j <= segments; j++ {
		...
		t = (float32(j) * dt) + angleStart
		a1 := x + rx*float32(math.Cos(float64(t)))
		b1 := y + ry*float32(math.Sin(float64(t)))
		c1 := -rx * float32(math.Sin(float64(t)))
		d1 := ry * float32(math.Cos(float64(t)))
